...
component jwt    
{
     ...
     */
     public string function encode( Required any payload, Required string key, Required string algo='HS256', struct appendClaims ) {
        try {
	        ...
	        claims[ "d" ] = Arguments.payload;
	        //claims['debug'] = true;
	        
	        if (StructKeyExists(arguments,'appendClaims')) {
	        	for (claim in appendClaims) {
	        		claims['#lcase(claim)#'] = appendClaims[claim];
	        	}
	        }
     ...
     public any function decode( Required string jwt, Required string key, Required boolean verify )
     {
     	...
     	/*WriteOutput('firing decode');
     	WriteDump(arguments);*/
        try
        {
            local.parts              = listToArray( Arguments.jwt, "." );
			//WriteDump(local.parts);
			...
			//WriteDump(urlsafeB64Decode( local.head64 ));
			//WriteDump(urlsafeB64Decode( local.body64 ));
            ...
            if ( Arguments.verify )
            {
				...
				//WriteOutput('with signinginput and testSignature');
				//WriteDump(local);
        ...
        catch( any e )
        {
			...
			switch( e.message )
            {
                case "invalidSegmentCount":
                    ...
                    throw("Wrong nubmber of segments");
                break;
                case "noAlgor":
                    //writeOutput( "No algorithm defined in header" );
                    throw("No algorithm defined in header");
                break;
